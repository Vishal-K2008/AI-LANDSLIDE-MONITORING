    ...
    <div className="p-3 sm:p-4 lg:p-8 max-w-7xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 bg-white p-4 sm:p-5 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">Tracking risk evolution: LOW → MODERATE → HIGH</p>
        </div>

        <select
          value={selectedLocId}
          onChange={(e) => setSelectedLocId(Number(e.target.value))}
          className="w-full sm:w-auto p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-800"
        >
          {locations.map(loc => (
      ...
      <div className="bg-white rounded-3xl p-4 sm:p-6 border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative">
          {progressionSteps.map((step, idx) => (
            <div key={idx} className="p-4 sm:p-5 rounded-2xl bg-slate-50 border border-slate-200 relative flex flex-col justify-between space-y-3">
              ...
              {idx < progressionSteps.length - 1 && (
                <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 md:bottom-auto md:left-auto md:-right-3 md:top-1/2 md:-translate-y-1/2 z-10 bg-white p-1 rounded-full border border-slate-200 shadow-sm">
        ...
        <div className="h-64 sm:h-72 w-full pt-2">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trendData}>
              ...
              <Line type="monotone" dataKey="risk" name="Risk Prob (%)" stroke="#ef4444" strokeWidth={3} />
