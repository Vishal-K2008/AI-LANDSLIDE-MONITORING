    ...
    <div className="p-3 sm:p-4 lg:p-8 max-w-7xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 bg-white p-4 sm:p-5 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">Real-time IoT sensor telemetry & satellite vegetation indices</p>
        </div>
        ...
        <select
          ...
          className="w-full sm:w-auto p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500"
        >
          {locations.map(loc => (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        {/* Rainfall */}
        <div className="p-3 sm:p-4 rounded-2xl bg-blue-50/70 border border-blue-200">
          ...
          <p className="text-xl sm:text-2xl font-bold font-display text-slate-900">{currentEnv ? currentEnv.rainfall_mm : 82} mm</p>
          <span className="text-[10px] text-blue-600 font-medium">Precipitation rate</span>
        </div>

        {/* Soil Moisture */}
        <div className="p-3 sm:p-4 rounded-2xl bg-teal-50/70 border border-teal-200">
          ...
          <p className="text-xl sm:text-2xl font-bold font-display text-slate-900">{currentEnv ? currentEnv.soil_moisture_pct : 78}%</p>
          <span className="text-[10px] text-teal-600 font-medium">Volumetric Content</span>
        </div>

        {/* Slope */}
        <div className="p-3 sm:p-4 rounded-2xl bg-amber-50/70 border border-amber-200">
          ...
          <p className="text-xl sm:text-2xl font-bold font-display text-slate-900">{selectedLoc ? selectedLoc.slope_deg : 34}°</p>
          <span className="text-[10px] text-amber-600 font-medium">Inclination</span>
        </div>
        ...
        <div className="p-3 sm:p-4 rounded-2xl bg-slate-100 border border-slate-200">
          ...
          <p className="text-xl sm:text-2xl font-bold font-display text-slate-900">{selectedLoc ? selectedLoc.elevation_m : 620} m</p>
          <span className="text-[10px] text-slate-500 font-medium">Above Sea Level</span>
        </div>

        {/* Humidity */}
        <div className="p-3 sm:p-4 rounded-2xl bg-purple-50/70 border border-purple-200">
          <div className="flex items-center gap-1.5 sm:gap-2 text-purple-700 text-[11px] sm:text-xs font-bold mb-1">
            <Wind className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />
            Humidity
          </div>
          <p className="text-xl sm:text-2xl font-bold font-display text-slate-900">{currentEnv ? currentEnv.humidity_pct : 91}%</p>
        ...
        <div className="p-3 sm:p-4 rounded-2xl bg-emerald-50/70 border border-emerald-200">
          <div className="flex items-center gap-1.5 sm:gap-2 text-emerald-700 text-[11px] sm:text-xs font-bold mb-1">
            <Satellite className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />
            Satellite Index
          </div>
          <p className="text-xl sm:text-2xl font-bold font-display text-slate-900">{currentEnv ? currentEnv.satellite_index : 0.85}</p>
          <span className="text-[10px] text-emerald-600 font-medium">NDMI Moisture</span>
        </div>
      </div>

      {/* Historical Telemetry Recharts */}
      <div className="bg-white rounded-3xl p-4 sm:p-6 border border-slate-200 shadow-sm space-y-3 sm:space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div>
            <h3 className="font-bold font-display text-slate-900 text-sm sm:text-base">Historical Changes Over Time (24h Telemetry)</h3>
            <p className="text-[11px] sm:text-xs text-slate-500">Multi-axis sensor stream tracking rainfall, soil moisture & temperature</p>
          </div>
          {isLoading && <RefreshCw className="w-4 h-4 text-teal-600 animate-spin shrink-0" />}
        </div>
