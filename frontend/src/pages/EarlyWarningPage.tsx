    ...
    <div className="p-3 sm:p-4 lg:p-8 max-w-7xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 bg-white p-4 sm:p-5 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">Categorized Alert Levels: LOW • MODERATE • HIGH • CRITICAL</p>
        </div>

        {currentRole === 'admin' && (
          <button
            onClick={() => setIsModalOpen(true)}
            className="flex items-center justify-center gap-2 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl shadow-md transition-all w-full sm:w-auto shrink-0"
          >
      ...
      <div className="space-y-4">
        ...
        {activeAlerts.length === 0 ? (
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 text-center space-y-2 text-slate-500">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl max-w-md w-full p-5 sm:p-6 border border-slate-200 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
